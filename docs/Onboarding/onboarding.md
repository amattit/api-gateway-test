...
### Экран онбординга
   #### **Пример UI:** 
   ![Картинка](/img/onboarding/onboarding1.png)
...
### Статус онбординга
- **GET** `/api/onboarding/v1/status`
- Проверяет, нужно ли пользователю проходить онбординг
- Возвращает `need_onboarding: boolean`

### Получение артистов
- **GET** `/api/onboarding/v1/artists`
- Возвращает список артистов для онбординга
- Поддерживает пагинацию через параметры `offset` и `limit`

### Поиск артистов
- **GET** `/api/onboarding/v1/artists/search`
- Поиск артистов по имени
- Параметр `query` для поискового запроса

### Похожие артисты
- **GET** `/api/onboarding/v1/artists/{artist_id}/similar`
- Возвращает список артистов, похожих на указанного
- `artist_id` - идентификатор артиста

### Завершение онбординга
- **POST** `/api/onboarding/v1/apply`
- Завершает процесс онбординга
- Принимает массив `artist_ids` с идентификаторами выбранных артистов

Подробная документация по API доступна в [Swagger](http://localhost:3000/api#tag/onboarding).
...
Для оптимизации производительности рекомендуется реализовать кэширование на стороне клиента:

- Кэшируйте результаты запросов к API артистов локально:
  - Сохраняйте данные артистов в локальное хранилище
  - Устанавливайте время жизни кэша (TTL) - рекомендуется 24 часа
  - При повторных запросах сначала проверяйте кэш
  - Обновляйте кэш только при истечении TTL

- Кэшируйте изображения артистов:
  - Сохраняйте URL и бинарные данные изображений
  - Используйте механизмы кэширования браузера
  - Очищайте кэш изображений при выходе из онбординга

### Тайм-ауты и обработка ошибок

- Установите тайм-ауты для всех сетевых запросов:
  - Рекомендуемое значение - 1500ms
  - При превышении тайм-аута показывайте сообщение об ошибке
  - Добавьте возможность retry запроса с back-off 75-150ms
  - Предоставьте возможность повторить запрос

- Обрабатывайте ошибки сети:
  - Показывайте понятные пользователю сообщения
  - Предлагайте альтернативные действия
  - Сохраняйте выбор пользователя при ошибках

### Защита от случайного выхода

- Предупреждайте пользователя при попытке покинуть страницу:
  - Показывайте диалог подтверждения
  - Информируйте о потере несохраненного прогресса
  - Предлагайте сохранить выбор перед выходом

- Сохраняйте состояние:
  - Периодически сохраняйте выбранных артистов
  - При возврате восстанавливайте последнее состояние
  - Очищайте сохраненное состояние только после успешного завершения

:::warning[Важно]
При реализации защиты от выхода учитывайте, что слишком частые предупреждения могут раздражать пользователя. Показывайте диалог подтверждения только если есть несохраненные изменения.
:::
